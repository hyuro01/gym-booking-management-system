  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryRef" :inline="true" v-show="showSearch" label-width="150px">
      ...
      <el-form-item label="用户性别" prop="sex" >
        <el-select v-model="queryParams.sex" placeholder="请选择用户性别" style="width: 200px"  clearable>
          <el-option
    ...
    <el-table v-loading="loading" :data="userinfoList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template #default="scope">
          <el-button type="primary" text @click="open1(scope.row)">查看</el-button>
        </template>
      </el-table-column>
...
import { listUserinfo, getUserinfo, delUserinfo, addUserinfo, updateUserinfo, listCoachinfo } from "@/api/manage/userinfo";
import { ElMessage, ElMessageBox } from 'element-plus'

const { proxy } = getCurrentInstance();
const { sys_user_sex, user_type } = proxy.useDict('sys_user_sex', 'user_type');
...
function getList() {
  loading.value = true;
  listCoachinfo(queryParams.value).then(response => {
    // 打印传入的条件
...
function handleExport() {
  proxy.download('manage/userinfo/export', {
    ...queryParams.value
